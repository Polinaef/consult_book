...
BOT_TOKEN = "8419017530:AAE5L-60pcl0xq77JMsHCczy_jC5ctP7Ta8"
SERVICE_ACCOUNT_FILE = "angelic-tracer-465313-t9-27151af1e09d.json"
SPREADSHEET_ID = "10nzYJJ0d8GU23hwWf_rzu6_GjgIFqegeJS_oMmVSm6o"
...
def client_type_keyboard():
    return ReplyKeyboardMarkup([[BUTTON_CONSULT], [BUTTON_BOOK], [BUTTON_QUESTION]], resize_keyboard=True,
                               one_time_keyboard=True)


CONTACT_KEYBOARD = ReplyKeyboardMarkup([[CONTACT_TELEGRAM, CONTACT_WHATSAPP]], resize_keyboard=True,
                                       one_time_keyboard=True)
YES_NO_KEYBOARD = ReplyKeyboardMarkup([[YES, NO]], resize_keyboard=True, one_time_keyboard=True)
DELIVERY_KEYBOARD = ReplyKeyboardMarkup([[DELIVERY_CDEK], [DELIVERY_YANDEX]], resize_keyboard=True,
                                        one_time_keyboard=True)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Здравствуйте! Выберите тему вашего обращения:",
...
async def handle_contact_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    if context.user_data.get("client_type") == "consult":
    ...
    elif context.user_data.get("client_type") == "book":
        context.user_data["contact_method"] = text
        context.user_data["book_step"] = 3
        await update.message.reply_text("Введите номер телефона:", reply_markup=ReplyKeyboardRemove())
    elif context.user_data.get("client_type") == "question":
        context.user_data["contact_method"] = text
...
async def handle_delivery_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("client_type") == "book" and context.user_data.get("book_step") == 4:
        ...
        try:
            ...
            await update.message.reply_text(f"Вы выбрали доставку: {name}. Хотите перейти на сайт {name}?",
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    if context.user_data.get("consult_step") == 2 or context.user_data.get("book_step") == 3 or context.user_data.get(
            "question_step") == 2:
        ...
        if context.user_data.get("client_type") == "consult":
            ...
            if context.user_data.get("in_moscow"):
                ...
                try:
                    ...
                    await update.message.reply_text("Произошла ошибка при сохранении данных. Попробуйте позже.")
                context.user_data.clear()
                await update.message.reply_text("Вы можете выбрать другие действия:",
                                                reply_markup=client_type_keyboard())
            else:
                context.user_data["book_step"] = 4
                await update.message.reply_text("Выберите службу доставки, которая вам удобна:",
...
async def handle_unknown_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("client_type"):
        ...
        if client_type == "consult":
            ...
            if step == 1:
                await update.message.reply_text("Пожалуйста, выберите способ связи, используя кнопки ниже:",
                                                reply_markup=CONTACT_KEYBOARD)
                return
        elif client_type == "book":
            step = context.user_data.get("book_step")
            if step == 1:
                await update.message.reply_text("Пожалуйста, выберите ответ, используя кнопки ниже:",
                ...
                await update.message.reply_text("Пожалуйста, выберите службу доставки, используя кнопки ниже:",
                                                reply_markup=DELIVERY_KEYBOARD)
                return
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex(f"^({BUTTON_CONSULT}|{BUTTON_BOOK}|{BUTTON_QUESTION})$"),
                                   handle_client_type_selection))
    app.add_handler(
        MessageHandler(filters.Regex(f"^({CONTACT_TELEGRAM}|{CONTACT_WHATSAPP})$"), handle_contact_selection))
